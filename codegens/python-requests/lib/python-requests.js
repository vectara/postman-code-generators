...
function getheaders (request, indentation) {
  ...
  if (!_.isEmpty(headerArray)) {
    ...
    // remove content type header if the type is formdata since
    // requests will add it properly.
    headerArray = headerArray.filter((header) => !(
      request.body && request.body.mode && request.body.mode === 'formdata'
